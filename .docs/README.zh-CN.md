...
> 由于浏览器可能会直接显示脚本内容，请**右键单击下方链接**，选择「另存为…」下载脚本文件。

[install.cmd](https://github.com/ahalpha/Snowbreak-AnitAmend/raw/refs/heads/master/install.cmd)

下载完成后，**双击运行**脚本，并根据屏幕提示完成操作。

#### 手动安装

1. **获取模组文件**  
   从 [Releases 页面](https://github.com/ahalpha/Snowbreak-AnitAmend/releases/latest) 下载你需要的模组。

2. **定位游戏安装目录**  
   - 如何找到游戏安装位置？  
     - **西山居启动器**：  
       点击「开始游戏」旁的齿轮图标 → 侧边栏选择「游戏」→ 点击「游戏安装目录」旁的「查看」按钮。  
     - **尘白禁区启动器**：  
       点击「开始游戏」旁的「☰」菜单（三条横线）→ 选择「安装位置」。

3. **创建模组文件夹**  
   依次打开以下路径：  
   `game` → `Game` → `Content` → `Paks`  
   然后在 `Paks` 文件夹内**新建**一个名为 `mods` 的文件夹。

4. **安装模组**  
   将第 1 步下载的模组文件复制或剪切到刚刚创建的 `mods` 文件夹中。
